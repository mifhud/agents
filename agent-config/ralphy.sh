...
3. Ensure the code works correctly"
  ...
  if [[ "$AUTO_COMMIT" == "true" ]]; then
    prompt+="
4. Commit your changes with a descriptive message using git-commit-workflow skill"
  fi
...
run_brownfield_task() {
  ...
  case "$AI_ENGINE" in
    claude)
      ...
      ;;
    opencode)
      OPENCODE_PERMISSION='{"*":"allow"}' opencode run --format json \
        ${MODEL_OVERRIDE:+--model "$MODEL_OVERRIDE"} \
...
$((step+1)). Commit your changes with a descriptive message using git-commit-workflow skill.
