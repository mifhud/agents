---
description: Run UAT (User Acceptance Testing) for HTTP/REST API, Queue/Topic messaging, or Scheduler tasks. Validates test cases against expected results and generates detailed reports.
...
- don't create file test, only test manually hit API, trigger scheduler task, or access via mcp database or message broker
- don't remove log files (if available)
