...
Delegates work to three skills: `sonarqube-code-smell-fetcher`, `sonarqube-code-smell-fixer`, and
`sonarqube-code-smell-validator`. Loads the `git-commit-workflow` skill when committing fixes.
...
Delegate to `sonarqube-code-smell-fetcher` with the user-provided URL. The fetcher returns
...
1. **Fix** — Delegate to `sonarqube-code-smell-fixer` with issue keys, file paths, line
   numbers, rule keys, and messages.
2. **Validate** — Delegate to `sonarqube-code-smell-validator`.
3. **Handle result:**
   - **APPROVED** — Load `git-commit-workflow` skill and commit. BLOCKER/CRITICAL get
